...
import { _bookingList, BOOKING_TYPE_OPTIONS, BOOKING_STATUS_OPTIONS } from 'src/_mock';
...
export function BookingListView() {
  ...
  const [tableData, setTableData] = useState(_bookingList);
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (booking) =>
        booking.bookingNumber.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        booking.spaceName.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        booking.renter.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
  ...
  if (type.length) {
    inputData = inputData.filter((booking) => type.includes(booking.bookingType));
  }
